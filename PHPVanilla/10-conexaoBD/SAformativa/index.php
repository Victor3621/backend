...
function listarLivros(PDO $pdo): array {
    $sql = "SELECT id, titulo, autor, preco, status, data_cadastro FROM livros ORDER BY id DESC";
    $stmt = $pdo->query($sql);
...
//tratamento de erros -> try executa e catchcaptura os erros
try {
    $pdo = ConexaoBanco::obterConexao(ARQUIVO_CONFIG);
    $conectado = TRUE;
    $livros = listarLivros($pdo);
} catch(PDOException $e){
    gravarLogErro("Erro no banco de dados: " . $e->getMessage());
    $mensagemErro = "Não foi possivel conectar ao banco de dados. tente mais tarde";
} catch (\Throwable $th) {
